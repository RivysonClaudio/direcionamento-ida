...
import SessaoCard from "../admin/sessoes/SessaoCard";
...
function SessoesList() {
  ...
  const [selectedAssistidoNome, setSelectedAssistidoNome] = useState(
    localStorage.getItem("member_sessao_filter_assistido_nome") || ""
  );
  const [selectedProfissionalNome, setSelectedProfissionalNome] = useState(
    localStorage.getItem("member_sessao_filter_profissional_nome") || ""
  );
  ...
  const [filter, setFilter] = useState(() => {
    const saved = localStorage.getItem("member_sessao_filter");
    return saved
      ? JSON.parse(saved)
      : {
          status: "",
  ...
  }, [selected, refreshTrigger, filter, database]);
  ...
  }, [isAssistidoDialogOpen, assistidoSearchTerm, database]);
  ...
  // Extrair ID do profissional da rota
  const getMemberId = () => {
  ...
  return (
    <div className="flex flex-col gap-3 h-full p-4 bg-(--blue)">
      ...
      <SeletorDeBotoes
        ...
        onChange={(value) => {
          setSelected(value);
          localStorage.setItem("member_sessao_selectedDay", value);
        }}
      />
            ...
            <button
              onClick={() => {
                ...
                localStorage.setItem(
                  "member_sessao_filter",
                  JSON.stringify(defaultFilter)
                );
                localStorage.removeItem("member_sessao_filter_assistido_nome");
                localStorage.removeItem(
                  "member_sessao_filter_profissional_nome"
                );
              ...
              assistidos.map((assistido) => (
                <button
                  ...
                  onClick={() => {
                    ...
                    if (newNome) {
                      localStorage.setItem(
                        "member_sessao_filter_assistido_nome",
                        newNome
                      );
                    } else {
                      localStorage.removeItem(
